...
const AILibraryPage = () => {
  ...
  return (
    ...
    >
      ...
      {/* 智能体列表 */}
      <SectionContainer
        title={`智能体列表 ${filteredAgents.length > 0 ? `(${filteredAgents.length})` : ''}`}
        description={filteredAgents.length === 0 ? "没有找到符合条件的智能体" : "选择并添加您需要的智能体"}
        backgroundClassName="bg-white"
        padding="standard"
      >
        {loading ? (
          <ResponsiveGrid xs={1} sm={2} md={3} gap="lg">
            {[1, 2, 3, 4, 5, 6].map((i) => (
              <Card key={i} className="overflow-hidden border border-gray-200/70 shadow-sm">
              ...
              return (
                ...
                >
                  <CardHeader className="pb-3">
                    <div className="flex justify-between items-start">
                      <div className="flex items-center gap-3">
                        <div className={`p-2 rounded-md bg-gradient-to-br ${categoryGradient} text-white`}>
                  ...
                  <CardFooter className="pt-1 pb-3 flex flex-wrap gap-2">
                    {agent.tags.map(tag => (
                      <Badge key={tag} variant="secondary" className="font-normal bg-gray-100/70">
                        {tag}
                      </Badge>
                    ))}
                  </CardFooter>
        ...
        {filteredAgents.length === 0 && !loading && (
          <div className="text-center py-12">
            ...
            <h3 className="text-xl font-medium text-gray-700 mb-1">没有找到智能体</h3>
            <p className="text-gray-500 mb-6">尝试调整搜索条件或选择不同的分类</p>
            <Button onClick={() => {
              setSearchQuery('');
              setCategory('all');
            }}>
