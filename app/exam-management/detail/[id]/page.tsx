...
export default function ExamDetailPage({ params }: DetailPageProps) {
  ...
  const examId = id || (routeParams?.id as string);
  ...
  if (!exam) {
    return (
      <div className="flex h-[50vh] flex-col items-center justify-center gap-4">
        ...
        <Button onClick={() => router.push("/exam-management")}>
          返回考试列表
        </Button>
      </div>
    );
    ...
    <div className="grid gap-6">
        ...
        <CardContent>
          <div className="grid gap-4 sm:grid-cols-2 md:grid-cols-3">
            ...
            <div>
              ...
              <p className="mt-1">
                {exam.startTime
                  ? format(new Date(exam.startTime), "yyyy-MM-dd HH:mm")
                  : "未设置"}
              </p>
            </div>
            <div>
              ...
              <p className="mt-1">
                {exam.endTime
                  ? format(new Date(exam.endTime), "yyyy-MM-dd HH:mm")
                  : "未设置"}
              </p>
            </div>
          </div>
      ...
      <Tabs defaultValue="questions">
        <TabsList>
          ...
          <TabsTrigger value="scores" disabled>
            学生成绩
          </TabsTrigger>
