...
// 获取指定考试的所有问题
export const getQuestionsByExamId = http.get(
...
// 创建新问题
export const createQuestion = http.post(
  "/api/questions",
  async ({ request }) => {
    await delay();
    ...
    if (
      ...
      return new Response(JSON.stringify({ message: "缺少必要字段" }), {
        status: 400,
    ...
    // 检查题号是否已存在于同一考试中
    const questionNumberExists = questions.some(
      (q) =>
        q.examId === body.examId && q.questionNumber === body.questionNumber
    );
...
export const updateQuestion = http.put(
  ...
  async ({ params, request }) => {
    ...
    if (questionIndex === -1) {
      return new Response(JSON.stringify({ message: "未找到要更新的题目" }), {
        status: 404,
        headers: { "Content-Type": "application/json" },
      });
    }
    ...
    if (
      body.questionNumber &&
      body.questionNumber !== questions[questionIndex].questionNumber
    ) {
      const questionNumberExists = questions.some(
        (q) =>
...
// 删除问题
export const deleteQuestion = http.delete(
  "/api/questions/:id",
  async ({ params }) => {
    await delay();
...
// 获取学业目标列表，可按学科筛选
export const getLearningObjectives = http.get(
...
// 获取考试详情（包括问题）
export const getExamDetails = http.get(
  "/api/exams/:id/details",
  async ({ params }) => {
    await delay();
...
// PDF导入题目
export const importQuestionsFromPdf = http.post(
  "/api/questions/import-pdf",
  async ({ request }) => {
    ...
    if (!examId) {
      return HttpResponse.json({ message: "缺少考试ID" }, { status: 400 });
    }
...
// 批量更新分数
export const bulkUpdateScores = http.post(
  "/api/questions/bulk-update-scores",
  async ({ request }) => {
