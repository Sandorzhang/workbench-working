...
const initExams = () => {
  ...
  const currentSemester = currentMonth >= 0 && currentMonth <= 6 ? '第二学期' : '第一学期'
  ...
  primarySchoolGrades.forEach((grade, gradeIndex) => {
    primarySubjects.forEach((subject, subjectIndex) => {
      ...
      const midtermDate = new Date(currentYear, midtermMonth - 1, 10 + gradeIndex % 5)
      ...
      const finalDate = new Date(currentYear, finalMonth - 1, 15 + gradeIndex % 5)
  ...
  middleSchoolGrades.forEach((grade, gradeIndex) => {
    ...
    gradeSubjects.forEach((subject) => {
      ...
      const midtermDate = new Date(currentYear, midtermMonth - 1, 12 + gradeIndex % 5)
      ...
      const finalDate = new Date(currentYear, finalMonth - 1, 10 + gradeIndex % 5)
      ...
      // 中考模拟 (只适用于初三)
      if (grade === '初三' && ['语文', '数学', '英语', '物理', '化学'].includes(subject)) {
  ...
  highSchoolGrades.forEach((grade, gradeIndex) => {
    highSubjects.forEach((subject) => {
      ...
      const midtermDate = new Date(currentYear, midtermMonth - 1, 15 + gradeIndex % 3)
      ...
      // 期末考试
      const finalMonth = currentSemester === '第一学期' ? 1 : 7
      const finalDate = new Date(currentYear, finalMonth - 1, 8 + gradeIndex % 5)
      ...
      // 高考模拟 (只适用于高三)
      if (grade === '高三' && ['语文', '数学', '英语', '理综', '文综'].includes(subject)) {
      ...
      if ((gradeIndex + subject.length) % 3 === 0) {
        const monthlyDate = new Date(currentYear, currentMonth, 20 + gradeIndex)
