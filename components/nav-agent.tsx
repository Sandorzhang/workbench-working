    ...
    const handleRemoveAgent = useCallback(
      async (agentId: string) => {
        try {
          ...
          // 在API调用前获取要移除的智能体信息
          const agentToRemove = localAgents.find(
          ...
          const response = await fetch(
            `/api/ai-library/user-agents/${agentId}`,
            {
              method: "DELETE",
            }
          );
      ...
      <>
        <SidebarGroup className="group-data-[collapsible=icon]:hidden">
          <div className="flex items-center justify-between pr-2">
            ...
            <Link
              href="/ai-library"
              className="flex h-7 w-7 items-center justify-center rounded-md text-sidebar-foreground/70 hover:bg-sidebar-hover"
            >
