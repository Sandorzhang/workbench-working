...
import { StudentRecord, RecordType, recordTypeNames, recordStatusNames, recordStatusColors } from '@/features/mentor-system/types';
...
export function StudentTracking({ studentId }: StudentTrackingProps) {
  ...
  const fetchRecords = async () => {
    try {
      ...
      // 使用新的API客户端获取学生记录
      const response = await api.mentorSystem.getStudentRecords(studentId);
      console.log("🔍 [StudentTracking] 获取学生记录响应:", response);
      
      if (response.success) {
        console.log("🔍 [StudentTracking] 获取到学生记录数量:", response.data.length);
        setRecords(response.data);
      } else {
        console.error("🔍 [StudentTracking] 获取学生记录失败:", response.message);
        toast({
          title: '获取记录失败',
      ...
      console.error("🔍 [StudentTracking] 获取学生记录出错:", error);
      toast({
        title: '获取记录失败', 
