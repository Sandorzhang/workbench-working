...
export function AppLayout({ children }: AppLayoutProps) {
  ...
  const { user } = useAuth();
  const [sidebarState, setSidebarState] = useState<boolean | undefined>(
    undefined
  );
  ...
  useEffect(() => {
    // 检查是否是学业标准详情页
    if (
      pathname.startsWith("/academic-standards/") &&
      pathname !== "/academic-standards"
    ) {
      // 获取标准详情以获取标题
    ...
    // 检查是否是数据资产详情页
      ...
      pathname.startsWith("/data-assets/") &&
      pathname !== "/data-assets"
    ) {
    ...
    // 对于学术旅程学生详情页
      ...
      pathname.startsWith("/academic-journey/students/") &&
      pathname !== "/academic-journey/students"
    ) {
      const fetchStudentDetail = async () => {
        try {
          const studentId = pathname.split("/").pop();
          const response = await fetch(
            `/api/academic-journey/students/${studentId}`
          );
          if (response.ok) {
            const data = await response.json();
  ...
  if (isSuperAdmin && pathname.startsWith("/superadmin")) {
    return (
      <SidebarProvider
        defaultOpen={sidebarState === undefined ? true : sidebarState}
      >
