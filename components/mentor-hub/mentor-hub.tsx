    ...
    const fetchMentors = async () => {
      try {
        ...
        if (!response.ok) {
          ...
          console.error(
            `获取导师数据失败 (状态: ${response.status}):`,
            errorText
          );
          throw new Error(
            `获取导师数据失败 (${response.status}): ${errorText}`
          );
        }
        ...
        setError(
          `获取导师数据失败: ${err instanceof Error ? err.message : "未知错误"}`
        );
      } finally {
        setIsLoading(false);
      }
              ...
              <div className="p-6">
                <MentorList
                  mentors={mentors.filter(
                    (mentor) => mentor.students && mentor.students.length > 0
                  )}
