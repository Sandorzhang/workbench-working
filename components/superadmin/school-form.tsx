  ...
  const loadGradesForType = async (type: string) => {
    try {
      ...
      const response = await fetch(
        `/api/school-grades?type=${encodeURIComponent(type)}`,
        {
          headers: {
      ...
      if (!response.ok) {
        throw new Error(
          `获取年级列表失败: ${response.status} ${response.statusText}`
        );
      }
            ...
            >
                  ...
                  <FormField
                    ...
                    render={({ field }) => (
                      <FormItem>
                        ...
                        <FormDescription>学校代码必须是3位数字</FormDescription>
                        <FormMessage />
                      </FormItem>
                    )}
                  />
              ...
              <div>
                ...
                <div className="border rounded p-4">
                  <p className="text-sm mb-3">根据所选学校类型自动配置的年级</p>
