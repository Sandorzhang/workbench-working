...
const generateTimelineMarkers = (filmStrips: FilmStrip[]) => {
  ...
  allDates.forEach((item) => {
    ...
    if (!monthlyMarkers[monthKey]) {
      monthlyMarkers[monthKey] = `${date.getFullYear()}年${
        date.getMonth() + 1
      }月`;
    }
  });
...
export const Timeline: React.FC<TimelineProps> = ({
  ...
  const [expandedMoment, setExpandedMoment] = useState<TeachingMoment | null>(
    null
  );
  ...
  useEffect(() => {
    const updateWidth = () => {
      if (timelineRef.current) {
        const parentWidth =
          timelineRef.current.parentElement?.offsetWidth || 800;
  ...
  const handleSelectSemester = (semester: string) => {
    ...
    // 滚动到对应的学期区域
    const stripIndex = filmStrips.findIndex(
      (strip) => strip.semester === semester
    );
            ...
            <div className="flex space-x-2">
              {filmStrips.map((strip) => (
                <Button
                  key={strip.semester}
                  variant={
                    selectedSemester === strip.semester ? "default" : "outline"
                  }
                  size="sm"
              ...
              >
                ...
                {filmStrips.map((strip, stripIndex) => {
                  ...
                  return strip.moments.map((moment, momentIndex) => {
                    // 计算每个时刻在当前学期区域内的相对位置
                    const momentPosition =
                      stripStart +
                      ((momentIndex + 1) / (strip.moments.length + 1)) *
                        stripWidth;
                ...
                <div className="aspect-video bg-black flex items-center justify-center">
                  <div className="relative w-full h-full">
                    ...
                    <div className="absolute inset-0 flex items-center justify-center">
                      <Button className="flex items-center gap-2 bg-primary hover:bg-primary/90 text-white rounded-full py-3 px-6 shadow-lg">
                    ...
                    >
                      <span className="flex items-center">
                        ...
                        <span className="ml-1.5">
                          {getTypeInfo(expandedMoment.type).label}
                        </span>
                      </span>
                    </Badge>
                    <Badge
                      ...
                      className={cn(
                        ...
                        subjectColors[expandedMoment.subject] ||
