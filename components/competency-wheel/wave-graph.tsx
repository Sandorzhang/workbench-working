...
export function WaveGraph(props: WaveGraphProps) {
  ...
  // 获取高亮索引
  const { hoverIndex, highlightAt, cancelHighlight } = useHighlightIndex(
    segments,
    selectedId
  );
  ...
  const labelPositions = useMemo(() => {
    ...
    segments.forEach((segment, index) => {
      ...
      if (rings.length > 0 && ringIndex >= 0 && ringIndex < rings.length) {
        ...
        console.log(
          `扇区${index} (${segment.label})使用环索引 ${ringIndex}，半径: ${rings[ringIndex].radius}`
        );
      } else {
        // 如果rings不存在或索引无效，使用基础半径
        outerRadii.push(radius * 0.35 + (effectiveLayer - 1) * layerSpacing);
        console.log(
          `扇区${index} (${segment.label})使用计算半径: ${
            radius * 0.35 + (effectiveLayer - 1) * layerSpacing
          }`
        );
      }
    });
  ...
  const renderArcPath = (
    ...
    if (Array.isArray(dashArray[0])) {
      ...
      const currentLayerDash =
        index < (dashArray as number[][]).length
      ...
      if (
        segmentIndex === 0 &&
        (index === 0 || index === dashArray.length - 1)
      ) {
        ...
        >
          <div
            style={{
              ...
              fontSize: "11px",
              fontWeight:
                selectedId === segment.id || hoverIndex === index
                  ? "bold"
                  : "normal",
              background:
